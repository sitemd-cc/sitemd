...
- `npx @sitemd-cc/sitemd init <dir>` now works on a fresh machine — previously the npm bin shim was skipped during package extraction because the native binary arrives later in `postinstall`, so `npx` failed with `sh: sitemd: command not found`; the package's `bin` now points at a committed Node wrapper at `sitemd/sitemd` that always exists at tarball-extraction time and execs the native binary at runtime
- `npm install @sitemd-cc/sitemd` now scaffolds `./sitemd/` with the full starter site (pages, settings, theme, binary), matching the `npx init` end state — adding sitemd to an existing project gives you a launchable dev server in one command instead of a two-step ritual, and the two install methods converge on the same project layout
- non-destructive install into existing projects — pre-existing `CLAUDE.md`, `AGENTS.md`, `.mcp.json`, and `.claude/skills/` / `.agents/skills/` are never overwritten; sitemd's agent context is appended inside `<!-- sitemd:begin --> ... <!-- sitemd:end -->` markers (edits inside the block survive re-installs), conflicting `.mcp.json` entries defer to `.mcp-sitemd.json`, and colliding skill directories defer to `<name>-sitemd/` — all deferred items surface in a console report at the end of install
- new `sitemd-merge` skill — agents can reconcile install conflicts automatically; ask your agent to "merge the sitemd install conflicts" after adding sitemd to a project with pre-existing agent content, and it walks the deferred files and merges or replaces per your preference
- native binary renamed on-disk from `sitemd/sitemd` to `sitemd/sitemd-bin`; user-facing command `./sitemd/sitemd launch` and `.mcp.json` paths (`"command": "./sitemd/sitemd"`) are unchanged — the file at that path is now a Node wrapper that resolves and execs the native binary, with automatic binary-download recovery if it's missing (e.g. when installed with `--ignore-scripts`)
