...
If `--output <dir>` is present in the arguments, this skill is being invoked by another workflow (typically the `release` skill) that wants page writes redirected into a staging directory instead of the live `pages/` tree. When this flag is set:

- All file writes (new pages and edits to existing pages) land under `<dir>/`, mirroring the relative path the file would normally have under `pages/`. For example, a doc page that would normally be created at `pages/docs/new-feature.md` is instead written to `<dir>/new-feature.md` (the `<dir>` itself already represents the docs subtree if used for `docs` type).
- When editing an existing page, first copy the live page from `pages/<path>` to `<dir>/<path>` if a staged copy doesn't already exist, then edit the staged copy in place.
- Do **not** call `sitemd_pages_create` in staged mode — it writes to the live tree and updates groups/nav. Write the file directly with the same frontmatter and body it would have produced.
- Do **not** call `sitemd_content_validate` in staged mode — validation happens when the staged file is promoted to the live tree by the calling workflow.
- Skip groups.md updates in staged mode for the same reason.
